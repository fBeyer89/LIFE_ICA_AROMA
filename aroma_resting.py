...
def create_aroma_prep(subject, working_dir, data_dir, freesurfer_dir, out_dir,
    ...
    'brain_mask' : 'preprocessed/'+subject+'/structural/T1_brain_mask.nii.gz', 
...
#
    ...
    aroma_prep.connect([
    (postprocess, apply_TF_denoised, [('outputnode.normalized_file', 'in_file')]),
    ...
    (apply_TF, sink,              [('out_file', 'transforms.withoutCSFWM.@denoised_result_MNI')]),
    (apply_TF_denoised, sink,              [('out_file', 'transforms.withCSFWM.@denoised_result_MNI')])
    ])
